#!/usr/bin/env python3
"""verify_m112.py — M112 (shm-link + 感知 + 动作 + 异常哨兵) 无头验证

QEMU(COM1=file 日志 / COM2=tcp:4000 模型链路 / monitor=4568) + shm 模型服务
(服务端经 monitor 注入 'os run hermes') -> 等待 M112 RESULT。
用法: python tools/verify_m112.py [--model qwen2.5:7b] [--timeout 360]
"""
import argparse
import os
import shutil
...
INITRD = os.path.join(ROOT, "sdk", "linux", "m112_ai.elf")
MON_PORT = 4568
LINK_PORT = 4000


def kill_peers():
    """清场: 所有 qemu + 本工具的 qwen_model_server.py (防僵尸实例互杀)。"""
...
def main():
    ...
    kill_peers()
    time.sleep(1.0)
    tmpd = tempfile.mkdtemp(prefix="m112-")
    ...
    print(f"[verify] booted (log={log})", flush=True)
    ...
    while time.time() < deadline:
        try:
            if os.path.exists(log):
                ...
                if "M112 RESULT: PASS" in txt:
                    result = "PASS"
                    break
                if "M112 RESULT: FAIL" in txt:
                    result = "FAIL"
                    break
        except OSError:
            pass
