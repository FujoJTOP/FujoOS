...
| **M6 darwinsubsys v0** | ✅ 已完成 | 内核 Mach-O 64 装载器(LC_SEGMENT_64 段映射+BSS 清零、LC_MAIN/LC_UNIXTHREAD 入口、运行时重定位至用户基址)、darwin BSD 空间 syscall 分发(write/exit/getpid, 0x2000000|nr)、格式嗅探三路路由(MZ→PE / ELF→ELF / Mach-O→darwin) | QEMU: 真实 Mach-O(MacOS ABI 样例, darwin write/exit) → 段映射 → ring3 执行 → `user : Mach-O program live — darwin bsd syscall write` → exit 接管; **三平台装载闭环**(ELF/PE/Mach-O 同出口); M2/M3/M4/M5 回归全绿 |
| **M9 模型原语 fujonn** | ✅ 已完成(实验分支 `fujoos-ai-dev`) | 内核 `ai.rs` 宏原生系统调用: 0x5101 意图分类 / 0x5102 fujoctx 上下文注入 / 0x5104 模型信息; `sdk/ai/agent.c` ring3 agent; 命令槽(0x402000)+宿主捕获 | QEMU: `m9 : agent cmd='run'` → `ai : classify('run') -> RUN` → fujoctx 注入 → agent 计划 → exit 内核接管 (提交 2f3576f) |
| **M10 Hermes CLI + Qwen 小模型** | ✅ 已完成(实验分支) | `sdk/hermes/hermes.c` ring3 交互 CLI (REPL: 0x5103 键盘读取); **engine=qwen**: 0x5101 → 内核 COM2 模型链路 (IRQ3/0x2F8, FJAI:REQ/RSP 行协议, hex) → 宿主 `tools/qwen_model_server.py` → **本地 Ollama qwen2.5:0.5b**; 超时→规则降级; **根因修复**: syscall 入口恢复全部 caller-saved 寄存器 (M9 intent=3/ctx[1883] 漂移 + M10 cr2=-3 #PF 同源) | QEMU+宿主: `classify('run') -> RUN [engine=qwen; model=qwen2.5:0.5b; t=64ms]` → fujoctx → tool 计划; REPL 键入 `open file` / `exit` 逐条走 Qwen 分类; 链路断开自动降级 rules (提交见 M10) |
