...
- [x] **M100** 发布:官网/文档/演示/公告
      - **docs/index.md 官网**: 特性亮点表 (内核/ABI/存储/图形/输入/
        音频/窗口/游戏/AI OS/工具链/交付) + 运行命令 + 文档索引;
      - **docs/49-release-notes.md 公告**: v1.0.0 (100 里程碑全部
        完成)、统计 (三 ABI/矩阵 9/9/CI 38 用例/onebuild 3/3/
        持久化/更新)、验收数字、架构蓝图、已知限制、下一步建议;
...
- [x] **M112** 三地基 + 异常哨兵 (首刀, A 职能) — 一次验收四件事
      ...
      - **实测 (Qwen 7b)**: hits=10 fp=0 (model 10/0/100), ev_anomaly=10,
        自动隔离 t1:2 → PASS → **[x] → docs/53**
- [x] **M113** 计划-执行器 (B) + I/O 预测器 (C) — 工具/序列组
      - B: 工具调用序列生成 (0x8301 → cap_exec), 计划→执行→验证→回滚;
        验收: "目标→步骤→执行→验证" 1 闭环
      - C: 行为序列预测 (预取下一文件/模块/编译); 验收: 命中率 vs LRU 30 轮
      - **实测 (Qwen 7b)**: plan1 2/0/1, plan2 cfg(1)=70, io 10 vs LRU 0 → PASS
        → **[x] → docs/54**
- [x] **M114** 自然语言配置 (D) + 环境侦察 (E) — 策略/感知组
      - D: 一句话 → 结构化策略对象 (0x81xx 能力授权); 验收: "工作时段禁游戏"生效
      - E: CPUID/传感器/设备树 → 机器类型/场景推断 → 自动适配; 验收: 适配生效
      - **实测 (Qwen 7b)**: policy applied=3 cfg=1/0/24, 执行面拒绝 -1,
        env desktop/profile=2 cfg(6)=2 → PASS → **[x] → docs/55**
- [x] **M115** Five-AI 回归 (docs/44 扩展 + 对照表)
      - 五职能集成回归 + 基线对照表 (docs/52 §5); 验收: 全部 PASS
      - **实测 (Qwen 7b)**: link=1, A 10/0, B 2/0/1, C 3 vs 0, D 3+-1,
        E 2/1 → **M115 RESULT: PASS** → **[x] → docs/56, Wave 7 完成**
